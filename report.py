...
${seo_head}
<script>(function(){try{var d=document.documentElement,p={};try{p=JSON.parse(localStorage.getItem("temperatury:appearance"))||{}}catch(e){}var os=window.matchMedia&&matchMedia("(prefers-color-scheme:dark)").matches?"dark":"light";d.setAttribute("data-dir",p.dir||"objective");d.setAttribute("data-theme",p.theme||os);d.setAttribute("data-density",p.density||"comfortable");d.setAttribute("data-hero",p.hero||"tint");d.setAttribute("data-unit",p.unit||(function(){var lg="";try{lg=localStorage.getItem("temperatury.lang")||""}catch(e){}lg=lg||d.getAttribute("lang")||"";/* English is the only unit-ambiguous language (spoken in Fahrenheit AND metric countries), so only then ask the browser's region: reading a Polish page must not default to F because a SECONDARY navigator.languages entry is en-US. */if(lg&&!/^en/i.test(lg))return"C";try{var L=navigator.languages||[],P=L[0]||navigator.language||"";/* ONLY the primary locale's region. Scanning the whole list gave a Polish visitor F because a SECONDARY entry was en-US; a tag with no region ("en", "pl") is not evidence of a Fahrenheit country. */var m=/-([A-Za-z]{2})(?:$$|-)/.exec(P);if(m)return["US","PR","GU","VI","AS","MP","BS","BZ","KY","PW","FM","MH"].indexOf(m[1].toUpperCase())>=0?"F":"C";}catch(e){}return"C";})());if(p.accent)d.setAttribute("data-accent",p.accent);if(p.font)d.setAttribute("data-font",p.font);if(/[?&]embed=1/.test(location.search))d.setAttribute("data-embed","1");}catch(e){}})();</script>
